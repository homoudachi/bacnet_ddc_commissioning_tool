...
- Repository state: **documentation plus Python CLIs** (`tools/`: simulator list verification, import compiler, runtime commissioning helpers including **flow/session inspection**, **run summary export** (optional embedded import/BIP JSON), **audited flow re-init**, **profile allowlisted BACnet writes** via **BACpypes3** when `--execute` is used, and **record-step** policy for failed/pending outcomes). Unit tests under `tests/`.
