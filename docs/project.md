...
- **Docker BACnet sim (beyond bacnet-dev)** — orchestrator-in-container, BBMD / macvlan lab parity (**deferred**—see post–baseline plan); **bacnet-dev** already runs **four** logical devices (three FCU + one HRV-shaped).
- **BACnet stack** — **PyInstaller exe** baseline shipped; **read/write batching**, **COV/subscribe**, and **sweep-timeouts** beyond the current [`CommissioningBACnetAdapter`](../tools/bacnet/adapter.py) façade.
