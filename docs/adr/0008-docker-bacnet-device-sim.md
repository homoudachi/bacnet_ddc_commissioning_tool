...
4. CI: **`docker_bacnet_smoke.sh`** → **`verify-bip-list --strict`**, **`bacnet-read`** spot checks (FCU **`ai_sat`** and **`av_supply_fan_command`** on all three FCU rows; HRV **`msv_test_mode`**, **`ai_supply_air_temperature`**, **`av_supply_fan_command`**, **`av_exhaust_fan_command`**), **`dry-run-bacnet-write --execute`** + read-back on **`msv_test_mode`** (FCU + HRV), **analog** writes + read-back on FCU supply fan AV / heat AV / valve AO and HRV fan command AVs, and **`bacnet-point-checkout`**, when Docker is available.
