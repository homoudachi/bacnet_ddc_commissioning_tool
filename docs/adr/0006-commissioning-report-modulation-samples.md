...
7. **`--output-pdf`**: same unified rows as a **server-generated** landscape PDF table (dependency: **fpdf2**); cell text truncated for layout stability. Optional **`--pdf-logo-image`** or run-dir file **`artifacts/branding/degree-logo.png`** for the raster logo; otherwise a small **vector** wordmark uses brand brown (**`#7C4200`**) and orange (**`#FF6000`**).
