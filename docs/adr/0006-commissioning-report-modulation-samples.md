...
7. **`--output-pdf`**: same unified rows as a **server-generated** landscape PDF table (dependency: **fpdf2**); cell text truncated for layout stability. Logo: **`--pdf-logo-image`**, or **`<run-dir>/artifacts/branding/logo.png`**, or the **neutral** checked-in **`docs/examples/branding/commissioning-logo-placeholder.png`**, or a minimal vector “site logo” hint box.
