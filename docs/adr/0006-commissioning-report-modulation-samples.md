...
7. **`--output-pdf`**: same unified rows as a **server-generated** landscape PDF table (dependency: **fpdf2**); cell text truncated for layout stability. Logo: **`--pdf-logo-image`**, or **`<run-dir>/artifacts/branding/logo.png`** ( **`init-run`** seeds this from the neutral **`docs/examples/branding/commissioning-logo-placeholder.png`** when absent), or that bundled path, or a minimal vector “site logo” hint box.
